...
from .serializers import EventSerializer, EventExportSerializer
...
def export_csv(webhook):
    ...
    data = [{'name': x.name, 'start': str(x.start), 'end': str(x.end)} for x in Event.objects.all()]
    keys = data[0].keys()
